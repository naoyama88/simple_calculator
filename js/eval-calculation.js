...
var evalCalculation = (function(String, pow, max) {
    ...
    var calcMultiplyOrDivide = (function(push) {
        return function calcMultiplyOrDivide(expression) {
            var multiplyOrDivide = /\*(-?(?:\d+(?:\.\d+)?|\.\d+))|\/(-?(?:\d+(?:\.\d+)?|\.\d+))|(-?(?:\d+(?:\.\d+)?|\.\d+))/g,
                multiply = [],
                divide = [],
                number,
                token,
        ...
        switch (operator) {
            case "+":
                powerNumber1 = pow(
                    10,
                    max(
                        getDecimalPartLength(number1),
                        getDecimalPartLength(number2)
                    )
                );
                result =
                    (powerNumber1 * number1 + powerNumber1 * number2) /
                    powerNumber1;
                break;
            case "-":
                powerNumber1 = pow(
            ...
            case "*":
                ...
                result =
                    (number1 * powerNumber1 * (number2 * powerNumber2)) /
                    (powerNumber1 * powerNumber2);
                break;
            case "/":
                powerNumber1 = pow(
                    10,
    ...
    return function evalCalculation(expression) {
        ...
        if (
            (illegalString = /^(?!-?(?:\d+(?:\.\d+)?|\.\d+)|\()[\s\S]/.exec(
                expression
            ))
        ) {
            throw new SyntaxError(
                "An expression starts with an unexpected token: " +
                    illegalString[0]
            );
        }
        ...
        if ((illegalString = /[^\d)]$/.exec(expression))) {
            throw new SyntaxError(
                "An expression ends with an unexpected token: " +
                    illegalString[0]
            );
        }
        ...
        if ((illegalString = /[^\d.+-]/.exec(expression))) {
            throw new SyntaxError(
                "An expression with an unexpected token: " + illegalString[0]
            );
        }
